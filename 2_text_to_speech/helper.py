...
class Helper:
    def preprocess_file_07_nvt(self, filename_07_nvt):
        ...
        if filename_arr_07_nvt[-1] != "mp3":
            filename_07_nvt += ".mp3"
        absolute_path_07_nvt = os_07_nvt.path.join(os_07_nvt.getcwd(), parent_dir_07_nvt + "sounds")
        files_07_nvt = [f for f in os_07_nvt.listdir(absolute_path_07_nvt) if os_07_nvt.path.isfile(os_07_nvt.path.join(absolute_path_07_nvt, f))]
        if filename_07_nvt in files_07_nvt:
            ...
            if action_07_nvt == 1:
                ...
                new_filename_07_nvt = filename_07_nvt.replace(".mp3", "") + "_" + datetime_07_nvt.datetime.now().strftime(format_datetime_07_nvt) + ".mp3"
                old_file_path_07_nvt = os_07_nvt.path.join(absolute_path_07_nvt, filename_07_nvt)
                new_file_path_07_nvt = os_07_nvt.path.join(absolute_path_07_nvt, new_filename_07_nvt)
                os_07_nvt.rename(old_file_path_07_nvt, new_file_path_07_nvt)
                print(f"Old file '{filename_07_nvt}' has been renamed to '{new_filename_07_nvt}'")
                return filename_07_nvt
        else:
            return filename_07_nvt
    ...
    def translate_text_07_nvt(self, text_07_nvt, input_lang_07_nvt, output_lang_07_nvt):
        ...
        translated_07_nvt = translator_07_nvt.translate(text_07_nvt, src=input_lang_07_nvt, dest=output_lang_07_nvt)
        return translated_07_nvt.text
    
    def speak_07_nvt(self, text_07_nvt, filename_07_nvt, input_lang_07_nvt, output_lang_07_nvt):
        script_directory_07_nvt = os_07_nvt.path.dirname(os_07_nvt.path.abspath(__file__))
        filename_07_nvt = self.preprocess_file_07_nvt(filename_07_nvt)
        print("Your file name is saving:", filename_07_nvt)
        filename_07_nvt = os_07_nvt.path.join(script_directory_07_nvt, "sounds/" + filename_07_nvt)
        text_07_nvt = self.translate_text_07_nvt(text_07_nvt, input_lang_07_nvt, output_lang_07_nvt)
